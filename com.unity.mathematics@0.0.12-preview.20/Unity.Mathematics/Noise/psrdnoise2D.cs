...
// A proper fix would involve umath.sing round() instead of math.math.floor() in selected
...
namespace Unity.Mathematics
{
    public static partial class noise
    {
        ...
        public static float3 psrdnoise(float2 pos, float2 per, float rot)
        {
            ...
            float3 xw = math.fmod(new float3(p0.x, p1.x, p2.x), per.x);
            float3 yw = math.fmod(new float3(p0.y, p1.y, p2.y), per.y);
